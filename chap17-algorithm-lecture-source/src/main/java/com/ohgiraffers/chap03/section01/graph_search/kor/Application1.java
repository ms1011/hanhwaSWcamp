...
public class Application1 {
    ...
    /* 설명. 문제에서 요구하는 입력 값을 받아 감염된 컴퓨터의 갯수(int/Integer)를 반환하는 메소드 작성 */
    public static Integer solution(String input) throws IOException {
        ...
        그래프정보가_닮긴_배열 = new int[node + 1][node + 1];      // node의 번호와 인덱스 위치를 일치시키기 위해서 크기를 1 증가(0번 인덱스는 사용 X)

        /* 설명. 방문 배열 작성(방문 배열을 작성하는 이유는 방문했던 노드를 다시 stack에 담지 않거나 다시 재귀 함수 호출을 하지 않기 위함) */
        방문 = new boolean[node + 1];          // 방문 배열도 노드 번호와 인덱스 번호를 일치 시키기 위해서 1 증가
...
//            map[a][b] = 1;
//            map[b][a] = 1;
            그래프정보가_닮긴_배열[a][b] = 그래프정보가_닮긴_배열[b][a] = 1;
        }
    ...
    public static void 깊이우선탐색_dfs(int start) {
        방문[start] = true;
        count++;

        /* 설명. 넘어온 노드와 연관된 노드를 찾는 반복문(기존에 방문하지 않았던) */
        for(int i = 1; i <= node; i++) {
            if(그래프정보가_닮긴_배열[start][i] == 1 && !방문[i])
                깊이우선탐색_dfs(i);
        }
    }
}
