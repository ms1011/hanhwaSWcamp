...
/* 설명. 데이터와 입출력을 위해 만들어지며 성공/실패 결과르 반환하는 클래스 */
public class MemberRepository {
    ...
    public MemberRepository() {
        ...
        /* 설명. 회원가입 기능 추가 후 파일이 기존에 존재하면(처음이 아니므로) 회원 3명으로 초기화 하기를 하지 않는다 */
        ...
        if(!file.exists()) {
            ...
            members.add(new Member(2, "user02", "pass02", 27, new String[]{"영화", "음악"}, BloodType.B));
            members.add(new Member(3, "user03", "pass03", 15, new String[]{"게임", "유튜브"}, BloodType.O));
...
//        System.out.println("===== repository에서 회원정보 더 읽어왔는지 확인 =====");
...
//        }
    ...
    private void loadMembers() {
        ...
        try {
            ...
            /* 설명. 파일로부터 모든 회원 정ㅂ를 읽어 memberList에 추가 (add) */
            while (true) {
...
//        return latestMember.getMemNo();                                     // 그 회원의 번호
    ...
    /* 설명. 기존 회원(객체)에 이어서 파일 출력을 하고 insert 한 객체의 수를 반환 (DML 작업의 결과는 int) */
    /* 설명. 객체 출력을 할 예정인데 기존 ObjectOutputStream 대신 새로 정의한 Stream 으로 회원 한명 출력해서 int 반환하기 (이어쓰기) */
