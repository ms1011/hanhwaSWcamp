...
public class MemberService {
    ...
    public void selectAllMembers() {
        ...
        /* 설명. 회원이 한명도 없어서 조회 결과가 없더라도 ArrayList 객체는 Empty 상태로 넘어온다 (null 체크가 아닌 isEmpty()를 써야하는 이유) */
        if(!selectedMembers.isEmpty()){         // 회원이 한명이라도 조회 된다면
            System.out.println("==== service까지 잘 반화되어 오나 확인 ====");
        ...
        /* 설명. 조건이 맞지 않아(회원이 조회되지 않아) 출력을 하는 구문 (위의 조건이 맞으면 실행되지 않음)(else 안쓰기) */
        System.out.println("아직 존재하는 회원이 없습니다");

    }

    /* 설명. 전달된 회원 번호를 활용해 repository에 있는 memberList로부터 해당 회원 찾아 반환하기 */
    ...
    /* 설명. 입력받아 넘어온 회원이 가질 번호를 만들고 추가 후 repository 전달 후 결과 확인 */
